...
void setup()
{
  ...
  // p0 = 104600; // Setting the ground level pressure fixed for  testing
  p0 = getPressure(); // Setting the ground level pressure
   lowpassFast = lowpassSlow = pressure;
  pinMode(3, INPUT); // Set our input  pins as such for altitude command input
}
...
void loop()
{
  ...
  time += 20;
   ...
   ch1 = pulseIn(3, HIGH, 25000); // Read the pulse width
  //   if(ch1>1000){
  ...
  //  }
  if((map(ch1, 1000,2000,-500,500)) < 0)
   {
    noTone(2);
    delay(750);
    if(hundreds == 0)
     {
      tone(2,900);                //long pulse if the number is  zero
      delay(600);
      noTone(2);
    }
    else
       for(char a = 0; a < hundreds; a++)          //this loop makes a beep for each  hundred meters
      {
        tone(2,900);
         delay(200);
        noTone(2);
     ...
     if(tens == 0)
    {
      ...
      for(char a = 0; a < tens; a++)          //this loop  makes a beep for each ten meters
      {
        tone(2,1100);
         delay(200);
        noTone(2);
    ...
    {
      pressure =  getPressure(); // warming up the sensor again
    } 
  } 
}
